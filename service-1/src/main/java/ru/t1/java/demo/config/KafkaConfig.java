...
public class KafkaConfig<T> {
    ...
    @Bean
    public ConsumerFactory<String, LogErrorOrMetrics> consumerLogErrorListenerFactory() {
        Map<String, Object> props = new HashMap<>();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, servers);
        props.put(ConsumerConfig.GROUP_ID_CONFIG, groupId);
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, JsonDeserializer.class);
        props.put(JsonDeserializer.VALUE_DEFAULT_TYPE, "ru.t1.java.demo.dto.LogErrorOrMetrics");
        props.put(JsonDeserializer.TRUSTED_PACKAGES, "*");
        props.put(JsonDeserializer.USE_TYPE_INFO_HEADERS, false);
        props.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, sessionTimeout);
        props.put(ConsumerConfig.MAX_PARTITION_FETCH_BYTES_CONFIG, maxPartitionFetchBytes);
    ...
    @Bean
    ConcurrentKafkaListenerContainerFactory<String, LogErrorOrMetrics> kafkaLogErrorListenerContainerFactory(@Qualifier("consumerLogErrorListenerFactory") ConsumerFactory<String, LogErrorOrMetrics> consumerFactory) {
        ConcurrentKafkaListenerContainerFactory<String, LogErrorOrMetrics> factory = new ConcurrentKafkaListenerContainerFactory<>();
        factoryBuilder(consumerFactory, factory);
        return factory;
    }

    @Bean
    ConcurrentKafkaListenerContainerFactory<String, AccountDto> kafkaAccountListenerContainerFactory(@Qualifier("consumerAccountListenerFactory") ConsumerFactory<String, AccountDto> consumerFactory) {
    ...
    @Bean
    ConcurrentKafkaListenerContainerFactory<String, TransactionDto> kafkaTransactionListenerContainerFactory(@Qualifier("consumerTransactionListenerFactory") ConsumerFactory<String, TransactionDto> consumerFactory) {
    ...
    @Bean
    ConcurrentKafkaListenerContainerFactory<String, TransactionResultDto> kafkaTransactionResultListenerContainerFactory(@Qualifier("consumerTransactionResultListenerFactory") ConsumerFactory<String, TransactionResultDto> consumerFactory) {
    ...
    @Bean
    public KafkaMetricProducer producerMetric(KafkaTemplate<String, Object> template) {
    ...
    @Bean
    public KafkaTransactionAcceptProducer producerTransactionAccept(KafkaTemplate<String, TransactionAcceptDto> template) {
